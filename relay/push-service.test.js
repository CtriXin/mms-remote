...
test("push service stores device registration and sends one completion alert", async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mms-remote-push-state-"));
  const sent = [];
...
test("push service defaults to a durable state file in the mms-remote home dir", () => {
  const resolved = resolvePushStateFilePath({
    CODEX_HOME: "/tmp/codex-home",
  });

  assert.equal(resolved, "/tmp/codex-home/mms-remote/push-state.json");
});
