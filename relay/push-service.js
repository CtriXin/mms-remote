...
// Purpose: Stores session-scoped APNs registration state and sends completion pushes for relay-hosted MMS Remote sessions.
...
function apnsConfigFromEnv(env) {
  return {
    teamId: readFirstDefinedEnv(["MMS_REMOTE_APNS_TEAM_ID"], env),
    keyId: readFirstDefinedEnv(["MMS_REMOTE_APNS_KEY_ID"], env),
    bundleId: readFirstDefinedEnv(["MMS_REMOTE_APNS_BUNDLE_ID"], env),
    privateKey: readAPNsPrivateKey(env),
  };
}

function readAPNsPrivateKey(env) {
  const rawValue = readFirstDefinedEnv(["MMS_REMOTE_APNS_PRIVATE_KEY"], env);
  if (rawValue) {
    return rawValue;
  }

  const filePath = readFirstDefinedEnv(
    ["MMS_REMOTE_APNS_PRIVATE_KEY_FILE"],
    env
  );
  if (!filePath) {
    return "";
...
function resolvePushStateFilePath(env = process.env) {
  ...
  return path.join(codexHome, "mms-remote", "push-state.json");
}
