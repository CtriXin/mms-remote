# Self-Hosting MMS Remote

This guide is for developers who clone the public GitHub repository and want to run MMS Remote on infrastructure they control.
...
- local pairing should work out of the box with `./run-local-mms-remote.sh`
- internet-facing manual bridge setups should pass their own relay URL explicitly with `MMS_REMOTE_RELAY`; source-checkout launcher setups can pass it with `./run-local-mms-remote.sh --relay-url`
...
## What MMS Remote Self-Hosting Means

MMS Remote is local-first.
...
- an iPhone with a MMS Remote build installed
...
Copy the generated `https://<random>.trycloudflare.com` URL, then start MMS Remote in another terminal:
...
- the MMS Remote relay

On your Mac:

- the MMS Remote bridge
...
- a shared-domain subpath, for example `wss://api.example.com/mms-remote/relay`
...
```sh
MMS_REMOTE_RELAY="wss://relay.example.com/relay" mms-remote up
```
...
npm install
MMS_REMOTE_RELAY="wss://relay.example.com/relay" npm start
```
...
If you install the bridge from npm and do not use the local launcher, make sure you export `MMS_REMOTE_RELAY` before running `mms-remote up`.
...
- you do not need push to use MMS Remote
...
- a bridge-side `MMS_REMOTE_PUSH_SERVICE_URL`
...
- only enable `MMS_REMOTE_TRUST_PROXY=true` when the proxy is trusted and sanitizes forwarded IP headers
...
- the bridge is using the correct `MMS_REMOTE_RELAY`
...
- point the bridge at your relay with `MMS_REMOTE_RELAY`
