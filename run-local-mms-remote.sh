...
# FILE: run-local-mms-remote.sh
# Purpose: Starts a local relay plus the public bridge for OSS and self-host workflows.
# Layer: developer utility
# Exports: none
# Depends on: node, npm, curl, relay/server.js, mms-remote-bridge/bin/mms-remote.js

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BRIDGE_DIR="${ROOT_DIR}/mms-remote-bridge"
...
log() {
  echo "[run-local-mms-remote] $*"
}

die() {
  echo "[run-local-mms-remote] $*" >&2
  exit 1
}

usage() {
  cat <<'EOF'
Usage: ./run-local-mms-remote.sh [options]
...
hold_open() {
  ...
  log "Press Ctrl+C to stop both the local relay and the MMS Remote bridge service."
  while true; do
    if [[ -n "${RELAY_PID}" ]] && ! kill -0 "${RELAY_PID}" 2>/dev/null; then
      wait "${RELAY_PID}"
      return $?
