...
function watchThreadRollout(threadId = "") {
  ...
  console.log(`[mms-remote] Watching thread ${resolvedThreadId}`);
  console.log(`[mms-remote] Rollout file: ${rolloutPath}`);
  console.log("[mms-remote] Waiting for new persisted events... (Ctrl+C to stop)");
...
function resolveThreadId(threadId) {
  ...
  throw new Error("No thread id provided and no remembered MMS Remote thread found.");
}
