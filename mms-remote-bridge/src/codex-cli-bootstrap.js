...
// Purpose: Detects, installs, and updates the global Codex CLI with transparent logs for MMS Remote installs and startup flows.
// Layer: CLI helper
...
const SKIP_BOOTSTRAP_ENV_NAME = "MMS_REMOTE_SKIP_CODEX_BOOTSTRAP";
...
function ensureCodexCLI({
  ...
  if (shouldSkipCodexBootstrap(env)) {
    logInfo(logger, `[mms-remote] Skipping Codex CLI bootstrap because ${SKIP_BOOTSTRAP_ENV_NAME}=1.`);
    return {
      status: "skipped",
      versionBefore: null,
      versionAfter: null,
  ...
  logInfo(logger, "[mms-remote] Checking Codex CLI...");
  ...
  if (versionBefore) {
    logInfo(logger, `[mms-remote] Codex CLI found (${versionBefore}).`);
  } else {
    logInfo(logger, "[mms-remote] Codex CLI not found.");
  }
  ...
  if (!npmVersion) {
    logWarn(
      logger,
      "[mms-remote] npm is unavailable, so MMS Remote could not install or update the Codex CLI automatically."
    );
    return {
      status: "failed",
      versionBefore,
  ...
  logInfo(logger, `[mms-remote] ${actionVerb} Codex CLI via npm (${CODEX_PACKAGE_SPEC})...`);
  ...
  try {
    ...
    logWarn(
      logger,
      `[mms-remote] Codex CLI ${versionBefore ? "update" : "install"} failed. ${message}`
    );
    return {
      status: "failed",
      versionBefore,
  ...
  if (!versionAfter) {
    logWarn(
      logger,
      "[mms-remote] Codex CLI bootstrap finished, but `codex --version` is still unavailable in this shell."
    );
    return {
      status: "failed",
      versionBefore,
  ...
  logInfo(
    logger,
    `[mms-remote] Codex CLI ${versionBefore ? "updated" : "installed"} (${versionAfter}).`
  );
  return {
