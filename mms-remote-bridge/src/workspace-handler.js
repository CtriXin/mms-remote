...
async function downsampleImageWithSips(imagePath, maxPixelDimension, timeoutMs = IMAGE_PREVIEW_TOOL_TIMEOUT_MS) {
  const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "mms-remote-image-preview-"));
...
async function createPatchSandbox(repoRoot, affectedFiles) {
  const sandboxRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "mms-remote-revert-preview-"));
...
async function initializePatchSandboxGitRepo(sandboxRoot) {
  ...
  await git(sandboxRoot, "config", "user.email", "mms-remote@example.local");
  await git(sandboxRoot, "config", "user.name", "MMS Remote");
  await syncPatchSandboxIndex(sandboxRoot);
...
async function createPatchBackup(repoRoot, affectedFiles) {
  const backupRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "mms-remote-revert-backup-"));
  const entries = [];
...
async function writeTempPatchFile(patchText) {
  const tempPatchPath = path.join(
    os.tmpdir(),
    `mms-remote-revert-${Date.now()}-${Math.random().toString(16).slice(2)}.patch`
  );
