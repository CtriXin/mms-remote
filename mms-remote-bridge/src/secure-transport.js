...
const HANDSHAKE_TAG = "mms-remote-e2ee-v1";
    ...
    if (!kind) {
      if (parsed.method || parsed.id != null) {
        sendControlMessage(createSecureError({
          ...
          message: "This bridge requires the latest MMS Remote iPhone app for secure pairing.",
        }));
        return true;
      }
      return false;
...
function debugSecureLog(message) {
  console.log(`[mms-remote][secure] ${message}`);
}
