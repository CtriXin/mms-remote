...
function createDesktopIpcClient({
  ...
  function ensureConnected() {
    ...
    nextSocket.on("connect", () => {
      isConnecting = false;
      sendRequest("initialize", { clientType: "mms-remote-bridge" })
        .then((result) => {
  ...
  function sendRequest(method, params) {
    ...
    const requestId = `mms-remote-${now().toString(36)}-${Math.random().toString(16).slice(2)}`;
    const envelope = {
      type: "request",
      requestId,
      sourceClientId: method === "initialize" ? "initializing-client" : clientId || "mms-remote-bridge",
