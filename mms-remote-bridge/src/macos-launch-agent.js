...
// Purpose: Owns macOS-only launchd install/start/stop/status helpers for the background MMS Remote bridge.
// Layer: CLI helper
// Exports: start/stop/status helpers plus the launchd service runner used by `mms-remote up`.
...
const {
  clearBridgeStatus,
  clearPairingSession,
  ensureMMSRemoteLogsDir,
  ensureMMSRemoteStateDir,
  readBridgeStatus,
  readDaemonConfig,
  readPairingSession,
  resolveBridgeStderrLogPath,
  resolveBridgeStdoutLogPath,
  resolveMMSRemoteStateDir,
  writeBridgeStatus,
  writeDaemonConfig,
  writePairingSession,
} = require("./daemon-state");

const SERVICE_LABEL = "com.mms.remote.bridge";
...
function runMacOSBridgeService({ env = process.env } = {}) {
  ...
  if (!config?.relayUrl) {
    ...
    console.error(`[mms-remote] ${message}`);
    return;
  }
...
async function startMacOSBridgeService({
  ...
  cliPath = path.resolve(__dirname, "..", "bin", "mms-remote.js"),
  ...
  ensureMMSRemoteStateDir({ env, fsImpl, osImpl });
  ensureMMSRemoteLogsDir({ env, fsImpl, osImpl });
...
function printMacOSBridgeServiceStatus(options = {}) {
  ...
  console.log(`[mms-remote] Service label: ${status.label}`);
  console.log(`[mms-remote] Installed: ${status.installed ? "yes" : "no"}`);
  console.log(`[mms-remote] Launchd loaded: ${status.launchdLoaded ? "yes" : "no"}`);
  console.log(`[mms-remote] PID: ${status.launchdPid || status.bridgeStatus?.pid || "unknown"}`);
  console.log(`[mms-remote] Bridge state: ${bridgeState}`);
  console.log(`[mms-remote] Connection: ${connectionStatus}`);
  console.log(`[mms-remote] Pairing payload: ${pairingCreatedAt}`);
  console.log(`[mms-remote] Stdout log: ${status.stdoutLogPath}`);
  console.log(`[mms-remote] Stderr log: ${status.stderrLogPath}`);
}
...
function writeLaunchAgentPlist({
  ...
  const stateDir = resolveMMSRemoteStateDir({ env, osImpl });
...
<dict>
  ...
  <dict>
    ...
    <key>MMS_REMOTE_DEVICE_STATE_DIR</key>
...
function assertRelayConfigured(config) {
  ...
  throw new Error("No relay URL configured. Run ./run-local-mms-remote.sh or set MMS_REMOTE_RELAY before enabling the macOS bridge service.");
}
