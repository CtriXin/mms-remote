...
// Purpose: Persists the latest active MMS Remote thread so the user can reopen it on the Mac for handoff.
// Layer: CLI helper
...
const STATE_DIR = path.join(os.homedir(), ".mms-remote");
...
function openLastActiveThread({ bundleId = DEFAULT_BUNDLE_ID } = {}) {
  ...
  if (!threadId) {
    throw new Error("No remembered MMS Remote thread found yet.");
  }
