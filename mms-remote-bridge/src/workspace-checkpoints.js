...
const CHECKPOINT_REFS_PREFIX = "refs/mms-remote/checkpoints";
...
async function captureGitCheckpoint(repoRoot, checkpointRef) {
  const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "mms-remote-checkpoint-"));
  const tempIndexPath = path.join(tempDir, `index-${process.pid}-${Date.now()}`);
  const env = {
    ...process.env,
    GIT_INDEX_FILE: tempIndexPath,
    GIT_AUTHOR_NAME: "MMS Remote",
    GIT_AUTHOR_EMAIL: "mms-remote@users.noreply.github.com",
    GIT_COMMITTER_NAME: "MMS Remote",
    GIT_COMMITTER_EMAIL: "mms-remote@users.noreply.github.com",
  };
  ...
  try {
    ...
    const message = `mms-remote checkpoint ref=${checkpointRef}`;
...
function validateCheckpointRef(checkpointRef) {
  ...
  if (!normalized.startsWith(`${CHECKPOINT_REFS_PREFIX}/`)) {
    throw workspaceCheckpointError(
      ...
      "Workspace checkpoint refs must stay inside the MMS Remote checkpoint namespace."
    );
  }
