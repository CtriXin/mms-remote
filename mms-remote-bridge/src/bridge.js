...
const RELAY_HISTORY_IMAGE_REFERENCE_URL = "mms-remote://history-image-elided";
...
function startBridge({
  ...
  createCodexTransport: explicitCreateCodexTransport = null,
} = {}) {
  const WebSocket = require("ws");
  const createCodexTransport = explicitCreateCodexTransport
    || require("./codex-transport").createCodexTransport;
  ...
  try {
    ...
    console.error(`[mms-remote] ${(error && error.message) || "Failed to load the saved bridge pairing state."}`);
    process.exit(1);
  }
  ...
  codex.onError((error) => {
    ...
    if (config.codexEndpoint) {
      console.error(`[mms-remote] Failed to connect to Codex endpoint: ${config.codexEndpoint}`);
    } else {
      console.error("[mms-remote] Failed to start `codex app-server`.");
      console.error(`[mms-remote] Launch command: ${codex.describe()}`);
      console.error("[mms-remote] Make sure the Codex CLI is installed and that the launcher works on this OS.");
    }
  ...
  function start() {
    ...
    try {
      ...
      nextChild.on?.("error", (error) => {
        consoleImpl.warn(`[mms-remote] Failed to hold the Mac awake while the bridge is active: ${error.message}`);
      });
      nextChild.on?.("exit", () => {
        if (child === nextChild) {
          child = null;
      ...
      consoleImpl.warn(
        `[mms-remote] Failed to start the bridge wake assertion: ${(error && error.message) || "unknown error"}`
      );
      child = null;
    }
  }
  ...
  for (const maxChars of [
    ...
    const compactedPayload = JSON.stringify({
      ...
      result: {
        ...
        mmsRemotePageCompactedForRelay: true,
      },
    });
...
function buildRelayHistoryCompactedThread(thread, turns, omittedTurnCount, keptTurnCount) {
  return {
    ...
    mmsRemoteHistoryCompacted: omittedTurnCount > 0,
    mmsRemoteOmittedTurnCount: omittedTurnCount,
    mmsRemoteKeptTurnCount: keptTurnCount,
  };
}
...
function buildRelayHistoryCompactionTurn(omittedTurnCount, keptTurnCount, idSource = {}) {
  ...
  return {
    id: `mms-remote-history-compacted-${baseId}`,
    mmsRemoteSynthetic: true,
    mmsRemoteHistoryCompacted: true,
    mmsRemoteOmittedTurnCount: omittedTurnCount,
    mmsRemoteKeptTurnCount: keptTurnCount,
    items: [
      {
        id: `mms-remote-history-compacted-item-${baseId}`,
