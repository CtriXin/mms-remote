...
const LEGACY_BRIDGE_DOWNGRADE_COMMAND = `npm install -g mms-remote@${LEGACY_BRIDGE_VERSION_FOR_IOS_1_0}`;
...
function buildLegacyIOSAppCompatibilityMessage({
  ...
  return `MMS Remote bridge ${normalizedBridgeVersion} requires MMS Remote iPhone `
    + `${MINIMUM_SUPPORTED_IOS_APP_VERSION} or later. `
    + `Install a matching iPhone build first, or install MMS Remote bridge `
...
function buildCachedIOSAppCompatibilityWarning({
  ...
  return formatNoticeBox({
    title: "!!! WARNING !!!",
    lines: [
      `MMS Remote bridge ${snapshot.bridgeVersion || "latest"} requires MMS Remote iPhone ${snapshot.minimumSupportedIOSAppVersion} or later.`,
      "Install a matching iPhone build first.",
      "",
