...
const DEFAULT_STATE_DIR_NAME = ".mms-remote";
...
// Reuses the existing MMS Remote state root so daemon mode keeps the same local-first storage model.
function resolveMMSRemoteStateDir({ env = process.env, osImpl = os } = {}) {
  return normalizeNonEmptyString(env.MMS_REMOTE_DEVICE_STATE_DIR)
    || path.join(osImpl.homedir(), DEFAULT_STATE_DIR_NAME);
}

function resolveDaemonConfigPath(options = {}) {
  return path.join(resolveMMSRemoteStateDir(options), DAEMON_CONFIG_FILE);
}

function resolvePairingSessionPath(options = {}) {
  return path.join(resolveMMSRemoteStateDir(options), PAIRING_SESSION_FILE);
}

function resolveBridgeStatusPath(options = {}) {
  return path.join(resolveMMSRemoteStateDir(options), BRIDGE_STATUS_FILE);
}

function resolveBridgeLogsDir(options = {}) {
  return path.join(resolveMMSRemoteStateDir(options), LOGS_DIR);
}
...
// Captures the last known service heartbeat so `mms-remote status` does not depend on launchctl output alone.
...
function ensureMMSRemoteStateDir({ fsImpl = fs, ...options } = {}) {
  fsImpl.mkdirSync(resolveMMSRemoteStateDir(options), { recursive: true });
}

function ensureMMSRemoteLogsDir({ fsImpl = fs, ...options } = {}) {
