...
async function gitCreatePullRequest(cwd, params, options = {}) {
  ...
  const bodyFile = path.join(os.tmpdir(), `mms-remote-pr-body-${process.pid}-${Date.now()}-${randomBytes(4).toString("hex")}.md`);
...
async function defaultFeatureBranchName(cwd, params) {
  const prefix = normalizeNonEmptyLine(params.featureBranchPrefix) || "mms-remote/mobile-pr";
...
function firstCommitMessageLine(message) {
  return normalizeNonEmptyLine(message) || "Changes from MMS Remote";
}
...
async function runStructuredCodexJson({
  ...
  const tempDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "mms-remote-git-ai-"));
...
async function stashChangesForWorktreeHandoff(cwd, pathspecArgs = []) {
  const stashLabel = `mms-remote-worktree-handoff-${randomBytes(6).toString("hex")}`;
  const output = await git(
    cwd,
    "stash",
    "push",
...
async function applyCopiedLocalChangesToWorktree(cwd, patch) {
  ...
  const patchFilePath = path.join(os.tmpdir(), `mms-remote-worktree-copy-${randomBytes(6).toString("hex")}.patch`);
...
function normalizeCreatedBranchName(rawName) {
  ...
  if (normalized.startsWith("mms-remote/")) {
    return normalized;
  }
  return `mms-remote/${normalized}`;
}
