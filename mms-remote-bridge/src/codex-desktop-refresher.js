...
class CodexDesktopRefresher {
  ...
  // Keeps one lightweight rollout watcher alive for the current MMS Remote-controlled thread.
...
function readBridgeConfig({
  ...
  const codexEndpoint = readFirstDefinedEnv(
    ["MMS_REMOTE_CODEX_ENDPOINT"],
    "",
    env
  );
  const refreshCommand = readFirstDefinedEnv(
    ["MMS_REMOTE_REFRESH_COMMAND"],
    "",
    env
  );
  const explicitRefreshEnabled = readOptionalBooleanEnv(["MMS_REMOTE_REFRESH_ENABLED"], env);
  const explicitKeepMacAwakeEnabled = readOptionalBooleanEnv(["MMS_REMOTE_KEEP_MAC_AWAKE"], env);
  ...
  return {
    ...
    pushPreviewMaxChars: parseIntegerEnv(
      readFirstDefinedEnv(["MMS_REMOTE_PUSH_PREVIEW_MAX_CHARS"], "160", env),
      160
    ),
    ...
    refreshDebounceMs: parseIntegerEnv(
      readFirstDefinedEnv(["MMS_REMOTE_REFRESH_DEBOUNCE_MS"], String(DEFAULT_DEBOUNCE_MS), env),
      DEFAULT_DEBOUNCE_MS
    ),
...
function isSourceCheckout(runtimeRoot, fsImpl) {
  ...
  return path.basename(runtimeRoot) === "mms-remote-bridge"
