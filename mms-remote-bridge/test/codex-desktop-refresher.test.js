...
test("readBridgeConfig keeps safe defaults and explicit overrides", () => {
  const macConfig = readBridgeConfig({
    env: {},
    platform: "darwin",
    runtimeRoot: "/tmp/mms-remote-package",
    fsImpl: {
      existsSync: () => false,
      readFileSync: () => {
  ...
  const persistedKeepAwakeConfig = readBridgeConfig({
    ...
    fsImpl: {
      existsSync(targetPath) {
        return targetPath === "/tmp/mms-remote-state/daemon-config.json";
      },
  ...
  const macEndpointConfig = readBridgeConfig({
    env: { MMS_REMOTE_CODEX_ENDPOINT: "ws://localhost:8080" },
    platform: "darwin",
    runtimeRoot: "/tmp/mms-remote-package",
    fsImpl: {
      existsSync: () => false,
  ...
  const linuxCommandConfig = readBridgeConfig({
    env: { MMS_REMOTE_REFRESH_COMMAND: "echo refresh" },
    platform: "linux",
    runtimeRoot: "/tmp/mms-remote-package",
    fsImpl: {
      existsSync: () => false,
  ...
  const explicitOnConfig = readBridgeConfig({
    env: {
      MMS_REMOTE_CODEX_ENDPOINT: "ws://localhost:8080",
      MMS_REMOTE_REFRESH_ENABLED: "true",
      MMS_REMOTE_DESKTOP_IPC_SOCKET: "/tmp/mms-remote-ipc.sock",
    },
    platform: "darwin",
    runtimeRoot: "/tmp/mms-remote-package",
    fsImpl: {
  ...
  const explicitOffConfig = readBridgeConfig({
    env: {
      MMS_REMOTE_REFRESH_COMMAND: "echo refresh",
      MMS_REMOTE_REFRESH_ENABLED: "false",
      MMS_REMOTE_KEEP_MAC_AWAKE: "false",
    },
    platform: "darwin",
    runtimeRoot: "/tmp/mms-remote-package",
    fsImpl: {
  ...
  assert.equal(explicitOnConfig.desktopIpcSocketPath, "/tmp/mms-remote-ipc.sock");
...
test("readBridgeConfig uses only the packaged relay default outside a source checkout", () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mms-remote-package-"));
...
test("readBridgeConfig preserves reverse-proxy subpaths when deriving push URLs", () => {
  const config = readBridgeConfig({
    env: {
      MMS_REMOTE_PUSH_SERVICE_URL: "https://relay.example/mms-remote",
    },
    runtimeRoot: "/workspace/mms-remote-bridge",
    fsImpl: {
      existsSync() {
  ...
  assert.equal(config.pushServiceUrl, "https://relay.example/mms-remote");
});
...
test("readBridgeConfig disables managed push defaults when a self-hosted relay override is set", () => {
  ...
  fs.writeFileSync(
    path.join(srcDir, "private-defaults.json"),
    JSON.stringify({ relayUrl: "wss://relay.example/mms-remote/relay" }),
    "utf8"
  );

  const config = readBridgeConfig({
    env: {
      MMS_REMOTE_RELAY: "wss://self-host.example/relay",
    },
    runtimeRoot: tempRoot,
    fsImpl: fs,
  });
