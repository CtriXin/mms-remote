...
test("ensureCodexCLI installs Codex when it is missing", () => {
  ...
  assert.deepEqual(messages, [
    "[mms-remote] Checking Codex CLI...",
    "[mms-remote] Codex CLI not found.",
    "[mms-remote] Installing Codex CLI via npm (@openai/codex@latest)...",
    "[mms-remote] Codex CLI installed (0.120.0).",
  ]);
});
...
test("ensureCodexCLI updates Codex when it is already installed", () => {
  ...
  assert.deepEqual(messages, [
    "[mms-remote] Checking Codex CLI...",
    "[mms-remote] Codex CLI found (0.118.0).",
    "[mms-remote] Updating Codex CLI via npm (@openai/codex@latest)...",
    "[mms-remote] Codex CLI updated (0.120.0).",
  ]);
});
...
test("shouldSkipCodexBootstrap respects the opt-out env flag", () => {
  assert.equal(shouldSkipCodexBootstrap({ MMS_REMOTE_SKIP_CODEX_BOOTSTRAP: "1" }), true);
  assert.equal(shouldSkipCodexBootstrap({ MMS_REMOTE_SKIP_CODEX_BOOTSTRAP: "true" }), true);
  assert.equal(shouldSkipCodexBootstrap({ MMS_REMOTE_SKIP_CODEX_BOOTSTRAP: "0" }), false);
});
