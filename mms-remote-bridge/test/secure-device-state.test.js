...
test("loadOrCreateBridgeDeviceState throws when the canonical file is corrupted and no fallback exists", () => {
  withTempDeviceStateEnv(({ canonicalStateFile }) => {
    ...
    assert.throws(
      () => loadOrCreateBridgeDeviceState(),
      /saved MMS Remote pairing state in device-state\.json is unreadable/i
    );
  });
});
...
function withTempDeviceStateEnv(run) {
  ...
  const previousMirror = process.env.MMS_REMOTE_DEVICE_STATE_KEYCHAIN_MOCK_FILE;
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mms-remote-device-state-"));
  ...
  process.env.MMS_REMOTE_DEVICE_STATE_DIR = tempRoot;
  process.env.MMS_REMOTE_DEVICE_STATE_KEYCHAIN_MOCK_FILE = keychainMirrorFile;
  ...
  try {
    ...
    if (previousMirror === undefined) {
      delete process.env.MMS_REMOTE_DEVICE_STATE_KEYCHAIN_MOCK_FILE;
    } else {
      process.env.MMS_REMOTE_DEVICE_STATE_KEYCHAIN_MOCK_FILE = previousMirror;
    }
...
function readCanonicalStateFromDisk() {
  const canonicalStateFile = path.join(process.env.MMS_REMOTE_DEVICE_STATE_DIR, "device-state.json");
