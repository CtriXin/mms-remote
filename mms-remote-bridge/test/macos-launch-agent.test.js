...
test("buildLaunchAgentPlist points launchd at run-service with mms-remote state paths", () => {
  const plist = buildLaunchAgentPlist({
    homeDir: "/Users/tester",
    pathEnv: "/usr/local/bin:/usr/bin",
    stateDir: "/Users/tester/.mms-remote",
    stdoutLogPath: "/Users/tester/.mms-remote/logs/bridge.stdout.log",
    stderrLogPath: "/Users/tester/.mms-remote/logs/bridge.stderr.log",
    nodePath: "/usr/local/bin/node",
    cliPath: "/tmp/mms-remote/bin/mms-remote.js",
  });

  assert.match(plist, /<string>com\.mms\.remote\.bridge<\/string>/);
  ...
  assert.match(plist, /<key>MMS_REMOTE_DEVICE_STATE_DIR<\/key>/);
});
...
test("resolveLaunchAgentPlistPath writes into the user's LaunchAgents folder", () => {
  assert.equal(
    ...
    path.join("/Users/tester", "Library", "LaunchAgents", "com.mms.remote.bridge.plist")
  );
});
    ...
    assert.deepEqual(calls, [
      [
        ...
        [
          "bootout",
          `gui/${process.getuid()}`,
          path.join(process.env.HOME, "Library", "LaunchAgents", "com.mms.remote.bridge.plist"),
        ],
      ],
      [
        "launchctl",
        [
          "bootout",
          `gui/${process.getuid()}/com.mms.remote.bridge`,
        ],
      ],
    ]);
  });
...
test("startMacOSBridgeService kickstarts the launch agent after bootstrap", () => {
  withTempDaemonEnv(({ rootDir }) => {
    ...
    const env = {
      ...process.env,
      HOME: rootDir,
      MMS_REMOTE_DEVICE_STATE_DIR: rootDir,
      MMS_REMOTE_RELAY: "ws://127.0.0.1:9000/relay",
    };
    ...
    assert.deepEqual(
      ...
      [
        ["launchctl", "bootout", `gui/${process.getuid()}`, path.join(rootDir, "Library", "LaunchAgents", "com.mms.remote.bridge.plist")],
        ["launchctl", "bootout", `gui/${process.getuid()}/com.mms.remote.bridge`, undefined],
        ["launchctl", "bootstrap", `gui/${process.getuid()}`, path.join(rootDir, "Library", "LaunchAgents", "com.mms.remote.bridge.plist")],
        ["launchctl", "kickstart", "-k", `gui/${process.getuid()}/com.mms.remote.bridge`],
      ]
    );
  });
});
...
test("getMacOSBridgeServiceStatus reports launchd + runtime metadata together", () => {
  withTempDaemonEnv(({ rootDir }) => {
    ...
    const plistPath = path.join(rootDir, "LaunchAgents", "com.mms.remote.bridge.plist");
    fs.mkdirSync(path.dirname(plistPath), { recursive: true });
    fs.writeFileSync(plistPath, "plist");

    const status = getMacOSBridgeServiceStatus({
      platform: "darwin",
      env: { HOME: rootDir, MMS_REMOTE_DEVICE_STATE_DIR: rootDir },
      execFileSyncImpl() {
        return "pid = 55";
      },
    });
...
function withTempDaemonEnv(run) {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "mms-remote-launch-agent-"));
