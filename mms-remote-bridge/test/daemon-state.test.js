...
test("daemon-state stores config, pairing payloads, and status under the mms-remote state dir", () => {
  withTempDaemonEnv(({ rootDir }) => {
    ...
    assert.equal(resolveMMSRemoteStateDir(), rootDir);
...
function withTempDaemonEnv(run) {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "mms-remote-daemon-state-"));
