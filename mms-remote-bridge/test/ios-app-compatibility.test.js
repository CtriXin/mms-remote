...
test("buildIOSAppCompatibilitySnapshot blocks iPhone 1.1 on bridge 1.3.9", () => {
  ...
  assert.equal(snapshot.downgradeCommand, "npm install -g mms-remote@1.3.7");
  assert.match(snapshot.message, /requires MMS Remote iPhone 1\.5 or later/i);
  assert.match(snapshot.message, /install MMS Remote bridge 1\.3\.7 to keep using iPhone 1\.1/i);
});
...
test("buildCachedIOSAppCompatibilityWarning warns when the last seen iPhone app is 1.1", () => {
  ...
  assert.match(warning, /requires MMS Remote iPhone 1\.5 or later/i);
  assert.match(warning, /Install a matching iPhone build first/i);
  assert.match(warning, /npm install -g mms-remote@1\.3\.7/i);
});
