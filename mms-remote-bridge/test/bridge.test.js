...
test("fetchAdaptiveThreadTurnsListForRelay expands small turns-list pages to the requested limit", async () => {
  ...
  assert.equal(
    response.result.data.some((turn) => turn.id.startsWith("mms-remote-history-compacted-")),
    false
  );
...
test("sanitizeThreadHistoryImagesForRelay replaces inline history images with lightweight references", () => {
  ...
  assert.deepEqual(content[1], {
    type: "image",
    url: "mms-remote://history-image-elided",
  });
});
...
test("sanitizeThreadHistoryImagesForRelay compacts oversized turns pages", () => {
  ...
  assert.equal(sanitized.result.mmsRemotePageCompactedForRelay, true);
  assert.deepEqual(
    sanitized.result.items.map((turn) => turn.id),
    ["turn-1"]
  );
  assert.equal(
    sanitized.result.items.some((turn) => turn.id.startsWith("mms-remote-history-compacted-")),
    false
  );
  assert.equal(sanitized.result.items[0].mmsRemotePageCompactedForRelay, true);
...
test("sanitizeThreadHistoryImagesForRelay preserves oversized turns pages instead of replacing them with a marker", () => {
  ...
  assert.equal(
    sanitized.result.data.some((turn) => turn.id.startsWith("mms-remote-history-compacted-")),
    false
  );
...
test("sanitizeThreadHistoryImagesForRelay compacts oversized history before the newest turn tail", () => {
  ...
  assert.equal(sanitized.result.thread.mmsRemoteHistoryCompacted, true);
  assert.equal(sanitized.result.thread.mmsRemoteOmittedTurnCount, 1);
  assert.equal(sanitized.result.thread.mmsRemoteKeptTurnCount, 1);
  assert.deepEqual(
    sanitized.result.thread.turns.map((turn) => turn.id),
    ["mms-remote-history-compacted-turn-old", "turn-new"]
  );
  assert.equal(
...
test("sanitizeThreadHistoryImagesForRelay keeps the newest forty turns when compacting", () => {
  ...
  assert.equal(sanitized.result.thread.mmsRemoteHistoryCompacted, true);
  assert.equal(sanitized.result.thread.mmsRemoteOmittedTurnCount, 5);
  assert.equal(sanitized.result.thread.mmsRemoteKeptTurnCount, 40);
  assert.deepEqual(
    sanitized.result.thread.turns.map((turn) => turn.id),
    [
      "mms-remote-history-compacted-turn-1",
