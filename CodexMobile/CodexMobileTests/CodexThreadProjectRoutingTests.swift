...
final class CodexThreadProjectRoutingTests: XCTestCase {
    ...
    func testMoveThreadToProjectPathKeepsRebindWhenResumeFailsOnlyBecauseRolloutIsMissing() async throws {
        ...
        let movedThread = try await service.moveThreadToProjectPath(
            threadId: "thread-1",
            projectPath: "/tmp/mms-remote-worktree"
        )

        XCTAssertEqual(resumeRequests.count, 1)
        XCTAssertEqual(resumeRequests.first?["threadId"]?.stringValue, "thread-1")
        XCTAssertEqual(resumeRequests.first?["cwd"]?.stringValue, "/tmp/mms-remote-worktree")
        XCTAssertEqual(movedThread.gitWorkingDirectory, "/tmp/mms-remote-worktree")
        XCTAssertEqual(service.thread(for: "thread-1")?.gitWorkingDirectory, "/tmp/mms-remote-worktree")
        XCTAssertEqual(service.currentAuthoritativeProjectPath(for: "thread-1"), "/tmp/mms-remote-worktree")
    ...
    func testManagedWorktreeAssociationPersistsAcrossLocalHandoffs() async throws {
        ...
        let worktreePath = "/Users/me/.codex/worktrees/a1b2/mms-remote"
        _ = try await service.moveThreadToProjectPath(threadId: "thread-1", projectPath: worktreePath)
        _ = try await service.moveThreadToProjectPath(threadId: "thread-1", projectPath: "/tmp/mms-remote-local")

        XCTAssertEqual(resumeResponses, [worktreePath, "/tmp/mms-remote-local"])
