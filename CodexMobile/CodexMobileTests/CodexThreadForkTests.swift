...
final class CodexThreadForkTests: XCTestCase {
    ...
    func testForkSendsOnlyThreadIdToThreadFork() async throws {
            ...
            case "thread/resume":
                ...
                return RPCMessage(
                    ...
                    result: .object([
                        "thread": .object([
                            "id": .string("fork-local"),
                            "cwd": .string("/tmp/mms-remote-worktree"),
        ...
        let forkedThread = try await service.forkThreadIfReady(
            from: "source-thread",
            target: .projectPath("/tmp/mms-remote-worktree")
        )
        ...
        XCTAssertEqual(capturedResumeParams["cwd"]?.stringValue, "/tmp/mms-remote-worktree")
        XCTAssertEqual(forkedThread.id, "fork-local")
        XCTAssertEqual(forkedThread.gitWorkingDirectory, "/tmp/mms-remote-worktree")
    ...
    func testUnsupportedThreadForkDisablesCapabilityAndShowsUpdatePrompt() async {
        ...
        do {
            ...
            XCTAssertEqual(service.bridgeUpdatePrompt?.title, "Update MMS Remote on your Mac to use /fork")
        }
    }
    ...
    func testForkKeepsAuthoritativeProjectPathWhenResumeFallsBackToMissingRollout() async throws {
        ...
        XCTAssertEqual(forkedThread.gitWorkingDirectory, "/tmp/mms-remote-worktree")
        XCTAssertEqual(service.currentAuthoritativeProjectPath(for: "fork-local"), "/tmp/mms-remote-worktree")
        ...
        XCTAssertEqual(service.thread(for: "fork-local")?.gitWorkingDirectory, "/tmp/mms-remote-worktree")
        XCTAssertEqual(service.currentAuthoritativeProjectPath(for: "fork-local"), "/tmp/mms-remote-worktree")
    ...
    func testLocalForkReturnsNilWhenWorktreeThreadHasNoLocalCheckout() {
        ...
        let worktreePath = tempRoot
            .appendingPathComponent(".codex/worktrees/a8b4/mmsremote-website", isDirectory: true)
