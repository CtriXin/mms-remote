...
final class SidebarThreadGroupingTests: XCTestCase {
    ...
    func testMakeGroupsMarksCodexManagedWorktreesInLabelAndIcon() throws {
        let now = Date(timeIntervalSince1970: 1_700_000_000)
        let threads = [
            makeThread(id: "main-thread", updatedAt: now, cwd: "/Users/me/work/MMS Remote"),
            makeThread(
                id: "worktree-thread",
                updatedAt: now.addingTimeInterval(-60),
                cwd: "/Users/me/.codex/worktrees/ce15/MMS Remote"
            ),
        ]

        let groups = SidebarThreadGrouping.makeGroups(from: threads, now: now)
        let mainGroup = try XCTUnwrap(groups.first(where: { $0.projectPath == "/Users/me/work/MMS Remote" }))
        let worktreeGroup = try XCTUnwrap(
            groups.first(where: { $0.projectPath == "/Users/me/.codex/worktrees/ce15/MMS Remote" })
        )

        XCTAssertEqual(mainGroup.label, "MMS Remote")
        XCTAssertEqual(mainGroup.iconSystemName, "folder")
        XCTAssertEqual(worktreeGroup.label, "MMS Remote 15")
    ...
    func testMakeProjectChoicesKeepWorktreeSelectionCompactWithoutShowingPathInLabel() {
        ...
        XCTAssertEqual(labelsByPath["/Users/me/work/MMS Remote"]?.label, "MMS Remote")
        XCTAssertEqual(labelsByPath["/Users/me/work/MMS Remote"]?.iconSystemName, "folder")
        XCTAssertEqual(labelsByPath["/Users/me/.codex/worktrees/ce15/MMS Remote"]?.label, "MMS Remote 15")
        XCTAssertEqual(labelsByPath["/Users/me/.codex/worktrees/ce15/MMS Remote"]?.iconSystemName, "arrow.triangle.branch")
    }
