...
final class TurnViewModelGitBranchWorktreeTests: XCTestCase {
    func testWorktreePathResolvesOnlyForBranchesCheckedOutElsewhere() {
        ...
        viewModel.gitBranchesCheckedOutElsewhere = ["mms-remote/feature-a"]
        viewModel.gitWorktreePathsByBranch = [
            "mms-remote/feature-a": "/tmp/mms-remote-feature-a",
            "main": "/tmp/mms-remote-main"
        ]

        XCTAssertEqual(
            viewModel.worktreePathForCheckedOutElsewhereBranch("mms-remote/feature-a"),
            "/tmp/mms-remote-feature-a"
        )
        XCTAssertNil(viewModel.worktreePathForCheckedOutElsewhereBranch("main"))
        XCTAssertNil(viewModel.worktreePathForCheckedOutElsewhereBranch("mms-remote/missing"))
    }
    ...
    func testApplyGitBranchTargetsStoresTrueLocalCheckoutPath() {
        ...
        let result = GitBranchesWithStatusResult(
            from: [
                ...
                "localCheckoutPath": .string("/tmp/mms-remote-local/mms-remote-bridge"),
                "current": .string("main"),
                "default": .string("main"),
            ]
        )
        ...
        XCTAssertEqual(viewModel.gitLocalCheckoutPath, "/tmp/mms-remote-local/mms-remote-bridge")
    }
    ...
    func testApplyGitBranchTargetsKeepsSelectedBaseBranchEmptyWhenDefaultIsRemoteOnly() {
        ...
        let result = GitBranchesWithStatusResult(
            from: [
                "branches": .array([.string("mms-remote/topic")]),
                "branchesCheckedOutElsewhere": .array([]),
                "worktreePathByBranch": .object([:]),
                "localCheckoutPath": .string("/tmp/mms-remote-local/mms-remote-bridge"),
                "current": .string("mms-remote/topic"),
                "default": .string("main"),
            ]
        )
    ...
    func testApplyGitBranchTargetsPreservesValidLocalBaseBranchSelection() {
        ...
        let result = GitBranchesWithStatusResult(
            from: [
                "branches": .array([.string("main"), .string("release/1.0"), .string("mms-remote/topic")]),
                "branchesCheckedOutElsewhere": .array([]),
                "worktreePathByBranch": .object([:]),
                "localCheckoutPath": .string("/tmp/mms-remote-local/mms-remote-bridge"),
                "current": .string("mms-remote/topic"),
