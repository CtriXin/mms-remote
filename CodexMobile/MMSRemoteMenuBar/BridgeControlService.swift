...
// Purpose: Wraps the existing mms-remote/npm shell commands so the menu bar app can detect the global CLI and control the bridge.
...
struct BridgeCLIInvocation {
    ...
    func command(_ arguments: [String]) -> String {
        ([shellQuoted(nodePath), shellQuoted(mmsRemotePath)] + arguments).joined(separator: " ")
    }
}
...
final class BridgeControlService {
    ...
    private let defaultStateDirectory = URL(fileURLWithPath: NSHomeDirectory())
        .appendingPathComponent(".mms-remote", isDirectory: true)
    private let launchAgentPlistURL = URL(fileURLWithPath: NSHomeDirectory())
        .appendingPathComponent("Library", isDirectory: true)
        .appendingPathComponent("LaunchAgents", isDirectory: true)
        .appendingPathComponent("com.mms.remote.bridge.plist")

    init(runner: ShellCommandRunner = ShellCommandRunner()) {
        self.runner = runner
    }

    // Confirms the product contract for this companion: a global `mms-remote` CLI must be runnable first.
    ...
    func updateBridgePackage() async throws {
        _ = try await runner.run(command: "npm install -g mms-remote@latest")
    }

    func fetchLatestPackageVersion() async -> Result<String, Error> {
        do {
            let result = try await runner.run(command: "npm view mms-remote version --json")
            let latestVersion = parseLatestVersion(result.stdout)
            guard let latestVersion else {
                throw BridgeControlError.commandFailed(
                    command: "npm view mms-remote version --json",
    ...
    private func loadFallbackSnapshot(
        ...
        return BridgeSnapshot(
            ...
            label: statusLines["service label"] ?? "com.mms.remote.bridge",
            platform: "darwin",
            installed: installed,
    ...
    private func resolveCLIInvocation() async throws -> BridgeCLIInvocation {
        let mmsRemotePath = try await resolveExecutable(named: "mms-remote")
        let nodePath = try await resolveNodePath(for: mmsRemotePath)
        return BridgeCLIInvocation(nodePath: nodePath, mmsRemotePath: mmsRemotePath)
    }
    ...
    private func resolveNodePath(for mmsRemotePath: String) async throws -> String {
        if let colocatedNodePath = resolveColocatedNodePath(for: mmsRemotePath) {
    ...
    private func resolveColocatedNodePath(for mmsRemotePath: String) -> String? {
        let mmsRemoteURL = URL(fileURLWithPath: mmsRemotePath)
        let candidateDirectories = [
            mmsRemoteURL.deletingLastPathComponent().path,
            mmsRemoteURL.resolvingSymlinksInPath().deletingLastPathComponent().path,
        ]
    ...
    private func resolveStateDirectory(statusLines: [String: String]) -> URL {
        if let explicitStateDirectory = normalizeNonEmptyString(ProcessInfo.processInfo.environment["MMS_REMOTE_DEVICE_STATE_DIR"]) {
    ...
    private func readLaunchAgentStateDirectory() -> URL? {
        guard let data = try? Data(contentsOf: launchAgentPlistURL),
              ...
              let stateDirectory = normalizeNonEmptyString(environment["MMS_REMOTE_DEVICE_STATE_DIR"] as? String) else {
            return nil
        }
    ...
    private func classifyCLIAvailability(from error: Error) -> BridgeCLIAvailability {
        ...
        if normalized.contains("command not found: mms-remote")
            || normalized.contains("mms-remote: command not found")
            || normalized.contains("mms-remote: not found")
