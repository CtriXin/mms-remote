...
enum BridgeMenuBarActionError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .missingCLI:
            return "Install the global `mms-remote` CLI before using this companion."
...
final class BridgeMenuBarStore: ObservableObject {
    ...
    private static let relayOverrideKey = "mms-remote.menuBar.relayOverride"
    ...
    func openLogsFolder() {
        ...
        let targetPath = (path?.isEmpty == false) ? path! : "\(NSHomeDirectory())/.mms-remote"
