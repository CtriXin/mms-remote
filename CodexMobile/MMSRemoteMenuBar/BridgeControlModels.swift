...
enum BridgeCLIAvailability: Equatable {
    ...
    static let installCommand = "npm install -g mms-remote@latest"
    ...
    var setupMessage: String {
        switch self {
        case .checking:
            return "Looking for a globally installed `mms-remote` command before enabling the companion controls."
        case .available(let version):
            return "Detected the global `mms-remote` CLI (`\(version)`)."
        case .missing:
            return "This companion only works when the global `mms-remote` CLI is installed and visible to the app shell environment."
        case .broken(let message):
            return "Found `mms-remote`, but the CLI could not be used. \(message)"
        }
    }
}
