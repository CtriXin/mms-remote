...
enum AppEnvironment {
    private static let defaultRelayURLInfoPlistKey = "MMS_REMOTE_DEFAULT_RELAY_URL"
    ...
    private static let supportEmailAddress = "mms-remote@example.invalid"
    ...
    // Legal links shown in the app. Replace these with your repository URLs
    // when publishing a signed build.
    static let privacyPolicyURL = URL(
        string: "https://github.com/CtriXin/mms-remote/blob/main/Legal/PRIVACY_POLICY.md"
    )!
    static let termsOfUseURL = URL(
        string: "https://github.com/CtriXin/mms-remote/blob/main/Legal/TERMS_OF_USE.md"
    )!
    ...
    static func feedbackMailtoURL(
        ...
        var queryItems = [
            URLQueryItem(name: "subject", value: "Share Feedback on MMS Remote with the Developer")
        ]
...
private extension AppEnvironment {
    ...
    static func feedbackBody(
        ...
        if let cliVersion = sanitizedCLIVersion(cliVersion) {
            lines.append("- MMS Remote CLI: \(cliVersion)")
        }
        lines.append("")
        lines.append("Notes:")
