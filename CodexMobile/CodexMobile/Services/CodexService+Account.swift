...
private let minimumBridgePackageUpdateCommand = "npm install -g mms-remote@latest"
private let forcedBridgeUpgradeFromVersion = "1.3.8"
private let forcedBridgeUpgradeTargetVersion = "1.3.9"
private let forcedBridgeUpgradeCommand = "npm install -g mms-remote@1.3.9"
...
extension CodexService {
    ...
    // Refreshes only the bridge package version state so MMS Remote updates stay independent from GPT UX.
...
extension CodexService {
    ...
    private func minimumBridgePackageUpdatePrompt(currentVersion: String?) -> CodexBridgeUpdatePrompt {
        ...
        if let currentVersion = currentVersion?.trimmingCharacters(in: .whitespacesAndNewlines),
           !currentVersion.isEmpty {
            message =
                "This computer bridge is running MMS Remote \(currentVersion), but this iPhone app requires MMS Remote \(CodexService.minimumSupportedBridgePackageVersion) or newer. Update the npm package on your computer, then reconnect."
        } else {
            message =
                "This computer bridge is too old for this version of MMS Remote iPhone. Update the MMS Remote npm package on your computer to \(CodexService.minimumSupportedBridgePackageVersion) or newer, then reconnect."
        }

        return CodexBridgeUpdatePrompt(
            title: "Update MMS Remote on your computer to reconnect",
            message: message,
    ...
    private func availableBridgePackageUpdatePrompt(
        ...
        CodexBridgeUpdatePrompt(
            title: "A newer MMS Remote update is available on your computer",
            message: "This computer bridge is running MMS Remote \(currentVersion), and npm now has MMS Remote \(latestVersion). Update the package on your computer when you're ready, then reconnect to start using the newer build.",
            command: minimumBridgePackageUpdateCommand
        )
    }
    ...
    private func forcedBridgePackageUpdatePrompt(currentVersion: String) -> CodexBridgeUpdatePrompt {
        CodexBridgeUpdatePrompt(
            title: "Update MMS Remote on your computer to reconnect",
            message: "This computer bridge is running MMS Remote \(currentVersion). Update the MMS Remote CLI on your computer to \(forcedBridgeUpgradeTargetVersion), then reconnect.",
    ...
    func isExpectedGPTLoginCallbackURL(_ url: URL) -> Bool {
        guard let callbackScheme = Bundle.main.object(
            forInfoDictionaryKey: "MMS_REMOTE_CHATGPT_CALLBACK_SCHEME"
        ) as? String else {
            return false
        }
