...
struct TurnTimelineView<EmptyState: View, Composer: View>: View {
    ...
    var body: some View {
        Group {
            if shouldShowEmptyTimeline {
                emptyTimelineContent
            } else {
                timelineScrollReaderContent
            }
        }
    }

    private var shouldShowEmptyTimeline: Bool {
        messages.isEmpty
            && !hasEarlierMessages
            && olderHistoryLoadErrorMessage == nil
            && !isLoadingRemoteEarlierMessages
    ...
    private var timelineScrollReaderContent: some View {
        ScrollViewReader { proxy in
            GeometryReader { viewport in
                timelineScrollContent(proxy: proxy, viewport: viewport)
            }
        }
    }

    private func timelineScrollContent(proxy: ScrollViewProxy, viewport: GeometryProxy) -> some View {
        let viewportWidth = viewport.size.width
        let contentWidth = timelineContentWidth(for: viewportWidth)
        let base = timelineScrollBase(contentWidth: contentWidth, viewportWidth: viewportWidth)
        let presented = timelineScrollPresentation(base, viewportWidth: viewportWidth)
        let withScrollCallbacks = timelineScrollCallbacks(presented, using: proxy)
        let withTimelineCallbacks = timelineStateCallbacks(withScrollCallbacks, using: proxy)
        return timelineLifecycle(withTimelineCallbacks, using: proxy)
    }

    private func timelineScrollBase(contentWidth: CGFloat, viewportWidth: CGFloat) -> some View {
        ScrollView(.vertical) {
            timelineRows(contentWidth: contentWidth, viewportWidth: viewportWidth)
            bottomAnchor(contentWidth: contentWidth, viewportWidth: viewportWidth)
        }
    }

    private func timelineScrollPresentation<Content: View>(
        _ content: Content,
    ...
    private func timelineScrollCallbacks<Content: View>(
        _ content: Content,
    ...
    private func timelineStateCallbacks<Content: View>(
        _ content: Content,
        using proxy: ScrollViewProxy
    ) -> some View {
        content
        .onChange(of: timelineChangeToken) { _, _ in
            handleTimelineChange(using: proxy)
        }
        ...
        .onChange(of: shouldAnchorToAssistantResponse) { _, newValue in
            handleAssistantAnchorChange(newValue: newValue, using: proxy)
        }
    }

    private func timelineLifecycle<Content: View>(
        _ content: Content,
        using proxy: ScrollViewProxy
    ) -> some View {
        content
    ...
    private func timelineRows(contentWidth: CGFloat, viewportWidth: CGFloat) -> some View {
    ...
    private func bottomAnchor(contentWidth: CGFloat, viewportWidth: CGFloat) -> some View {
        Color.clear
    ...
    private func scrollBottomGeometry(from geometry: ScrollGeometry) -> ScrollBottomGeometry {
    ...
    private func handleAssistantAnchorChange(newValue: Bool, using proxy: ScrollViewProxy) {
        if newValue {
