...
// Keep rich-text selection out of the scrolling timeline. This is shared by both
...
@MainActor
private enum CachingMarkdownParser {
    ...
    static func attributedString(for input: String) -> AttributedString {
        ...
        let result = NativeMarkdownRenderer.attributedString(for: input)
...
private enum NativeMarkdownRenderer {
    static func attributedString(for input: String) -> AttributedString {
        ...
        return (try? AttributedString(markdown: input, options: options)) ?? AttributedString(input)
    }
}
