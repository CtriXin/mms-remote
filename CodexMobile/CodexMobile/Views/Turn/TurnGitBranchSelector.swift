...
func mmsRemoteNormalizedCreatedBranchName(_ rawName: String) -> String {
    let trimmedName = rawName.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !trimmedName.isEmpty else { return "" }
    if trimmedName.hasPrefix("mms-remote/") {
        return trimmedName
    }
    return "mms-remote/\(trimmedName)"
}

// Leaves "open elsewhere" branches selectable so the caller can surface the right alert or git error.
func mmsRemoteCurrentBranchSelectionIsDisabled(
    branch: String,
...
func mmsRemoteSelectableDefaultBranch(
...
struct TurnGitBranchSelector: View, Equatable {
    ...
    private var localDefaultBranch: String? {
        ...
        return mmsRemoteSelectableDefaultBranch(
...
struct TurnGitBranchPickerSheet: View {
    ...
    private var isNewBranchNameValid: Bool {
        ...
        guard !trimmed.isEmpty, trimmed != "mms-remote/" else { return false }
        return true
    }
    ...
    private var suggestedCreateBranchName: String? {
        ...
        let candidate = mmsRemoteNormalizedCreatedBranchName(searchText)
        ...
        List {
            Section(sectionTitle) {
                if showsDefaultBranchRow, let defaultBranch {
                    let isDisabled = mmsRemoteCurrentBranchSelectionIsDisabled(
            ...
            if allowsSelectingCurrentBranch {
                Section {
                    ...
                    Button {
                        let fromSearch = mmsRemoteNormalizedCreatedBranchName(searchText)
...
#Preview("Branch Picker") {
    TurnGitBranchPickerSheet(
        branches: [
            "feature/auth-flow",
            "feature/dark-mode",
            "mms-remote/onboarding-v2",
            "mms-remote/sidebar-refactor",
            "fix/crash-on-launch",
        ...
        selectedBranch: "mms-remote/onboarding-v2",
        defaultBranch: "main",
        currentBranch: "mms-remote/onboarding-v2",
