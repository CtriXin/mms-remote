...
private let qrScannerBridgeUpdateCommand = "npm install -g mms-remote@latest"
...
func validatePairingQRCode(_ code: String, now: Date = Date()) -> QRScannerPairingValidationResult {
    ...
    if looksLikeMMSRemotePairingPayload(data) {
        return .bridgeUpdateRequired(
            makeScannerBridgeUpdatePrompt(
                message: "This QR code looks like it came from an older MMS Remote bridge. Update the npm package on your computer to the latest release before scanning a new QR code."
            )
        )
    }

    return .scanError("Not a valid secure pairing code. Make sure you're scanning a QR from the latest MMS Remote bridge.")
}
...
private func looksLikeMMSRemotePairingPayload(_ data: Data) -> Bool {
