...
[![npm version](https://img.shields.io/npm/v/mms-remote)](https://www.npmjs.com/package/mms-remote)
[![License](https://img.shields.io/badge/License-Apache--2.0-blue.svg)](LICENSE)

Control [Codex](https://openai.com/index/codex/) from your iPhone. MMS Remote is a local-first open-source bridge + iOS app that keeps the Codex runtime on your Mac and lets your phone connect through a paired secure session.
...
Build the iOS app from source in Xcode, install your own signed build on-device, then use the in-app onboarding flow to pair by scanning the QR from `mms-remote up`.
...
│  MMS Remote iOS │ ◄────────────────────► │ mms-remote (Mac) │ ◄──────────────────────► │ codex       │
...
1. Run `mms-remote up` on your Mac
2. On macOS, MMS Remote installs/starts a lightweight background bridge service and prints a QR for first-time pairing or recovery
3. Scan the QR once with the MMS Remote iOS app to trust that Mac
...
```
├── mms-remote-bridge/                # Node.js bridge package used by `mms-remote`
│   ├── bin/                      # CLI entrypoints
...
- **A signed MMS Remote iOS build** installed on your iPhone or iPad before scanning the pairing QR
...
If you plan to use the macOS menu bar companion, `mms-remote` must be installed globally and available in your login-shell `PATH`.

```sh
npm install -g mms-remote@latest
```
...
If you only want to try MMS Remote, you can install it from npm and run it without cloning this repository.
...
On first connect, open the MMS Remote app, follow the onboarding flow, then scan the QR code from inside the app.
...
```sh
git clone <your-mms-remote-repo-url>
cd mms-remote
./run-local-mms-remote.sh
```
...
- `./run-local-mms-remote.sh --hostname <lan-hostname-or-ip>`
- `./run-local-mms-remote.sh --relay-url https://<random>.trycloudflare.com`
- `./run-local-mms-remote.sh --bind-host 127.0.0.1 --port 9100`
...
If you want the npm bridge to point at your own setup instead of the package default, override `MMS_REMOTE_RELAY` explicitly:
...
3. Set `MMS_REMOTE_RELAY` to the Tailscale-reachable `ws://` or `wss://` relay URL
4. Pair once with QR
...
```sh
MMS_REMOTE_RELAY="wss://api.example.com/mms-remote/relay" mms-remote up
```

In that setup, the public endpoints can look like this:

- `wss://api.example.com/mms-remote/relay`
- `https://api.example.com/mms-remote/v1/push/session/register-device`
- `https://api.example.com/mms-remote/v1/push/session/notify-completion`

Have the proxy strip `/mms-remote` before forwarding so the relay still receives `/relay/...` and `/v1/push/...`.

If you point `MMS_REMOTE_RELAY` at your own self-hosted relay, managed push stays off unless you also set `MMS_REMOTE_PUSH_SERVICE_URL` on the bridge and explicitly enable push on the relay.
...
On macOS, `mms-remote up` is the friendly entrypoint for the background bridge service:
...
On non-macOS platforms, `mms-remote up` runs the bridge in the foreground.
...
### `mms-remote reset-pairing`

Clears the saved bridge pairing state so the next trusted connection requires a fresh QR bootstrap again.
You normally do not need this for corrupted local state anymore: recent MMS Remote builds auto-repair unreadable pairing files/mirrors on startup.

```sh
mms-remote reset-pairing
# => [mms-remote] Cleared the saved pairing state. Run `mms-remote up` to pair again.
```
...
```sh
mms-remote resume
# => [mms-remote] Opened last active thread: abc-123 (phone)
```

### `mms-remote watch [threadId]`
...
`MMS_REMOTE_RELAY` is optional, but the default depends on how you got MMS Remote:
...
- if you are running from source, assume you should use `./run-local-mms-remote.sh` or set `MMS_REMOTE_RELAY` yourself
...
| `MMS_REMOTE_REFRESH_DEBOUNCE_MS` | `1200` | Debounce window (ms) for coalescing refresh events |
| `MMS_REMOTE_REFRESH_COMMAND` | — | Custom shell command to run instead of the built-in AppleScript refresh |
| `MMS_REMOTE_CODEX_BUNDLE_ID` | `com.openai.codex` | macOS bundle ID of the Codex app |
| `CODEX_HOME` | `~/.codex` | Codex data directory (used here for `sessions/` rollout files) |

```sh
# Enable desktop refresh explicitly
MMS_REMOTE_REFRESH_ENABLED=true mms-remote up

# Connect to an existing Codex instance
MMS_REMOTE_CODEX_ENDPOINT=ws://localhost:8080 mms-remote up
...
MMS_REMOTE_RELAY="wss://relay.example/relay" \
MMS_REMOTE_PUSH_SERVICE_URL="https://relay.example" \
mms-remote up
```

On the relay/VPS side, keep push disabled until you actually want it. The HTTP push endpoints are off by default and only turn on when you set `MMS_REMOTE_ENABLE_PUSH_SERVICE=true`.

## Pairing and Safety

- MMS Remote is local-first: Codex, git operations, and workspace actions run on your Mac, while the iPhone acts as a paired remote control.
...
- The bridge state lives canonically in `~/.mms-remote/device-state.json` with local-only permissions. On macOS the bridge also mirrors that state to Keychain as best-effort backup/migration data, and recent builds auto-repair unreadable local state on startup instead of requiring manual cleanup.
- The CLI no longer prints the connection URL in plain text below the QR.
- Set `MMS_REMOTE_RELAY` only when you want to self-host or test locally against your own setup.
- Leave `MMS_REMOTE_TRUST_PROXY` unset for direct/self-hosted installs. Turn it on only when a trusted reverse proxy such as Traefik, Nginx, or Caddy is forwarding the relay traffic.
...
- A fresh QR scan can replace the previously trusted iPhone automatically. Use `mms-remote reset-pairing` only when you intentionally want to wipe the remembered pairing state yourself.
...
To make that limitation more practical, MMS Remote also includes a hand-off button in the iPhone app. It lets you explicitly continue the current chat on your Mac by opening the matching thread in `Codex.app` when you are ready to switch devices.

**Known limitation**: The Codex desktop app does not live-reload when an external `app-server` process writes new data to disk. Threads created or updated from your phone won't appear in the desktop app until it remounts that route. MMS Remote keeps desktop refresh off by default for now because the current deep-link bounce is still disruptive. You can still enable it manually if you want the old remount workaround.
...
This triggers a debounced deep-link bounce (`codex://settings` → `codex://threads/<id>`) that forces the desktop app to remount the current thread without interrupting any running tasks. While a turn is running, MMS Remote also watches the persisted rollout for that thread and issues occasional throttled refreshes so long responses become visible on Mac without a full app relaunch. If the local desktop path is unavailable, the bridge self-disables desktop refresh for the rest of that run instead of retrying noisily forever.
...
Not for MMS Remote itself. You need Codex CLI set up and working independently.
...
Run `mms-remote reset-pairing`, then start the bridge again with `mms-remote up`. You should only need this when you intentionally want to replace the paired iPhone or wipe the remembered pairing.

**Can I connect to a remote Codex instance?**
Yes — set `MMS_REMOTE_CODEX_ENDPOINT=ws://host:port` to skip spawning a local `codex app-server`.

**Why don't my phone threads show up in the Codex desktop app immediately?**
The desktop app reads session data from disk (`~/.codex/sessions`) but doesn't live-reload when an external process writes new data. Your phone still gets the live stream; it is the desktop GUI that lags unless you explicitly enable the refresh workaround with `MMS_REMOTE_REFRESH_ENABLED=true`.

**Does MMS Remote support true live sync between phone and `Codex.app`?**
...
Yes. That is the intended forking path. The transport and push-service code are in [`relay/`](relay/); point `MMS_REMOTE_RELAY` at the instance you run.

**Can I use Tailscale?**
Yes. It is the recommended private-network option for self-hosting on iPhone. Run your relay somewhere reachable over Tailscale, set `MMS_REMOTE_RELAY` to that relay URL, pair once with QR, then let the app reconnect to the trusted Mac through the same relay.
...
The MMS Remote name, marks, and branding are not licensed for use in forks or derivative projects. If you fork, redistribute, or publish a modified version, use a different app name and branding.
