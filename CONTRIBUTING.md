...
- **iPhone** with the MMS Remote app (or built from source)
...
npm install
MMS_REMOTE_RELAY="ws://localhost:9000/relay" npm start
```

That runs `mms-remote up`, which:
1. Spawns a Codex `app-server` process
...
Scan the QR code with the MMS Remote iOS app to trust that Mac.
...
1. Start the local launcher: `./run-local-mms-remote.sh`
...
For OSS/local development, prefer the launcher above. If you want to point the bridge process at your own relay manually without the launcher, export `MMS_REMOTE_RELAY` in your shell:

```sh
# Connect to an existing Codex instance instead of spawning one
MMS_REMOTE_CODEX_ENDPOINT=ws://localhost:8080 npm start

# Use your own self-hosted relay endpoint (`ws://` is unencrypted)
MMS_REMOTE_RELAY="ws://localhost:9000/relay" npm start

# Enable auto-refresh of Codex.app on Mac
MMS_REMOTE_REFRESH_ENABLED=true npm start
```
...
```
mms-remote/
├── mms-remote-bridge/          # Node.js CLI bridge (npm package)
│   ├── bin/mms-remote.js      # CLI entrypoint
│   └── src/
...
│       ├── session-state.js        # Thread persistence (~/.mms-remote/)
...
- Set `MMS_REMOTE_RELAY` to a relay you control when you are not using the local launcher, or pass the relay URL to the launcher with `--relay-url`. Use `wss://` when you want TLS in transit.
- MMS Remote uses an authenticated end-to-end encrypted transport after pairing completes. The relay code is public for inspection, but deployed relay details should stay in private config.
