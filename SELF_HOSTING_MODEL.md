...
This file explains what the public MMS Remote repository is for, what it includes, and what it does not include.

If you cloned MMS Remote from GitHub, the intended path is local-first usage or self-hosting on infrastructure you control.
...
1. Local LAN pairing on your own machine with `./run-local-mms-remote.sh`
2. A self-hosted relay on your own VPS, passed in through `MMS_REMOTE_RELAY`
...
- people can run MMS Remote locally
...
If you fork or self-host MMS Remote, keep these things out of the public repo:
...
If you cloned MMS Remote from GitHub:

- do not expect a private hosted relay to be built in
- use `./run-local-mms-remote.sh` for local testing
- use `MMS_REMOTE_RELAY` for your own VPS or hosted relay
